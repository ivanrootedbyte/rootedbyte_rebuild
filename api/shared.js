...
module.exports = async function handler(req, res) {
  ...
  try {
    const query = new URLSearchParams({
      share_slug: `eq.${id}`,
      select: "share_slug,app_type,title,input_summary,output_json,is_public,expires_at,created_at",
      limit: "1"
    });

    const response = await fetch(`${supabaseUrl}/rest/v1/shared_outputs?${query.toString()}`, {
      method: "GET",
      headers: {
    ...
    return sendJson(res, 500, {
      error: error.message || "Something went wrong retrieving the shared output."
    });
  }
};
